...
function KellyColorPicker(cfg) {
    ...
    function rgbToHex(color, color2, color3) {
    ...
    function constructor(cfg) {
        ...
        if (cfg.input) {
            ...
            var inputEdit = function (e) {
                ...
                handler.setColorByValue(e.target.value);
            };
    ...
    this.setColorByValue = function (inputValue) {
        var tmp;
        if (tmp = /^rgba\(([\d]+), ?([\d]+), ?([\d]+), ?(0.[\d]+)/.exec(inputValue)) {
            inputValue = rgbToHex(tmp[1], tmp[2], tmp[3]);
            a = parseFloat(tmp[4]);
        }

        if (!inputValue || !inputValue.length) return;

        if (inputValue.charAt(0) != '#') inputValue = '#' + inputValue;

        if (inputValue.length != 7) return;
        else if (!inputValue.match(/^#([0-9A-F]){3}$|^#([0-9A-F]){6}$/img)) return;

        if (hex && inputValue == hex && rendered) return;
